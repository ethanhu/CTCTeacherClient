...
public class DistrictTrainSearchLayout {

	private static SynClientSupport synClientSupport;
	private Shell shell;
	DistrictTrainData districtStationData = new DistrictTrainData();

	public DistrictTrainSearchLayout(){}
	public DistrictTrainSearchLayout(Shell shell, MinaClient minaClient){
	...
	public Composite create(Composite parentComp) {
		...
		createGroup(comp,"查询区段内所定义车次信息","pos 0.5al 0.2al",null);
		return  comp;
	}
	...
	private void createComp(Composite group) {
	    ...
	    OKButton.addSelectionListener(new SelectionAdapter() {
	    	public void widgetSelected(final SelectionEvent e) {
	    		...
	    		String districtstationID = districtStation.getText();
	    	
				if (getDistrictStationInfo(districtstationID)){
					new DistrictTrainBrowserLayout(DistrictTrainSearchLayout.this.shell,
													 DistrictTrainSearchLayout.this.synClientSupport,
													 districtstationID).show();
				}
				else
					showMsg("该区段内还没有定义任何车次信息!");
	    	}
	    });
	    OKButton.setText("查询");
	...
	//从服务器获取District信息
	private boolean getDistrictStationInfo(String districtTrainID) {
		...
		Object[] params = new Object[]{districtTrainID};
		...
		if(listString == null){//此情况不会出现
			...
			List<TrainDistrictRelation> list = JsonUtil.getList4Json(listString,TrainDistrictRelation.class);
			if(list.size() <= 0)
				return false;
			else{

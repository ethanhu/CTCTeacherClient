...
public class UtilForStatics {
	...
	public void putScheduleTrainMap(Plan plan) {
		...
		String arriveTime = plan.getPlan_arrivestationtime();//到站时间  时间格式同数据库中的格式： 00：00：00
		...
		int timeType = Constants.TDCS_TIME_TYPE_ARRIVEAL;//车次的时间类别 6291457/Constants.TDCS_TIME_TYPE_LEAVE
		...
		//目前暂时设为：false ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
		boolean operatorFlag = false;//可操作标记 : false可以操作, true不能再操作: 根据车次的运行时间与当前时间的比对决定
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public void appendScheduleTrainMap(LineFigure newTrainLine){
		...
		String operator = CTCTeacherMain.getUserName();//操作者
		...
		if( sourceAnchor != null)
		{
			...
			value = new TrainPlan(prestationName,stationName,trainName,time,trainDirection,
					                operatorFlag,trainType,districtName,operator);
			
			//System.out.println("K01:"+trainName+"/K02:"+ stationName+"/K03:"+timeType);
			//System.out.println("01:"+prestationName+"/02:"+ stationName+"/03:"+trainType+"/03:"+time);
		...
		if( targetAnchor != null)
		{
			...
			value = new TrainPlan(prestationName,stationName,trainName,time,trainDirection,
					                operatorFlag,trainType,districtName,operator);
			//System.out.println("K001:"+trainName+"/K002:"+ stationName+"/K003:"+timeType);
			//System.out.println("001:"+prestationName+"/002:"+ stationName+"/003:"+trainType+"/003:"+time);
			//System.out.println("*****************************");
		...
		//System.out.println("添加:到站:前站:"+prestationName+"/当前站:"+stationName+"/时间类别:"+timeType+"/数量:"+scheduleTrainMap.size());
	...
	private String xyTimeToStr(int  x){
		String time = "";
		int curX = UtilForTimeRectangle.xToTime(x);//将屏幕时间转换为实际时间如屏幕值为：117 ,转换后为：20分钟
	...
	private void deleteScheduleTrainMap(TrainStationTime key){
		...
		if (scheduleTrainMap.containsKey(key)){//含有要删除的记录
			...
			//System.out.println("01:"+trainType+"/"+value.getStationName()+"/"+value.getTrainName()+"/"+timeType);
		...
		//System.out.println("001前站:"+prestationName+"/当前站:"+stationName+"/时间类别:"+timeType+"/数量:"+scheduleTrainMap.size());
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public void adjustScheduleTrainMap(LineAnchorFigure newTrainLine, int newX){
		...
		int trainType = newTrainLine.getTrainType();//车次类别，新加的，从数据库读的等  对原计划车次进行调整 ...
		String trainName = newTrainLine.getTrainName();//车次信息
		...
		case Constants.TDCS_TRAIN_TYPE_NEW_SAVE://对新添加已保存过的车次进行调整
			value.setTrainType(Constants.TDCS_TRAIN_TYPE_NEW_SAVE_ADJUST);//对新添加,并保存过的线,进行调整
			value.setTime(time);
			break;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// Map<TrainStation_x,TrainPlan_x> scheduleTrainMap = new HashMap<TrainStation_x,TrainPlan_x>();
	//获取满足要求的所有车次信息
	public Map<TrainStation,PlanForStatics> getStationTrainsMap(String stationName,String trainName,int directionFlag){
		...
		for(TrainStationTime key : scheduleTrainMap.keySet()){
			...
			)
			{
				...
				if( (directionFlag == 2) || (directionFlag == direction))//上行0，下行1 ,双向2
				{
					...
					if (timeType == Constants.TDCS_TIME_TYPE_LEAVE) //58 离站
					{
	...
	private int deleteAllOrNot(String trainName,int trainType){
		...
		if ( ! scheduleTrainMap.isEmpty()){
			for(TrainStationTime key : scheduleTrainMap.keySet()){
				TrainPlan	value = scheduleTrainMap.get(key);
				if ( (value.getTrainName()).equalsIgnoreCase(trainName) &&
					 (value.getTrainType() != trainType) )
				{
					number++;
				}
			}//for		
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	/*
 	   1.对于教师的操作：更新Train TrainDistrictRelation  Plan，
 	   2. 对于学员的操作：更新Train TrainDistrictRelation  Dispatch
 	         这里的操作 对象是教师，即只进行（1）的处理
	*/
	...
	public boolean saveToDB() {
		...
		Map<TrainStation,PlanForStatics> insertPlanMap = new HashMap<TrainStation,PlanForStatics>();//用于更新Plan表 插入
		...
		for(TrainStation key : stationTrainsMap.keySet()){
			...
			{ 
				...
				case Constants.TDCS_TRAIN_TYPE_NEW://新添加的车次信息36
					...
					break;
				case Constants.TDCS_TRAIN_TYPE_NEW_SAVE_DEELETE://对已经保存过，又进行删除的新添加车次
					if(deleteAllOrNot(trainName,Constants.TDCS_TRAIN_TYPE_NEW_SAVE_DEELETE) == 0)//不存在, 即可以进行删除
					{
				...
				case Constants.TDCS_TRAIN_TYPE_DELETE://删除原计划车次35
					...
					if(deleteAllOrNot(trainName,Constants.TDCS_TRAIN_TYPE_DELETE) == 0)//不存在, 即可以进行删除
					{
		...
		//都空的话，不进行操作
		if (! (insertTrainDistrictMap.isEmpty()&& 
			  deleteTrainDistrictMap.isEmpty()&& 
			  insertPlanMap.isEmpty()&&
			  updatePlanMap.isEmpty()) )
		{
			//写数据到数据库
			resultFlag = databaseServer.batchUpdateFromTDCSForTeacher(insertTrainDistrictMap,deleteTrainDistrictMap,
					                         insertPlanMap,updatePlanMap,firstStation,lastStation);

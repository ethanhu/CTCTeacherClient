...
import ctc.tdcs.command.ErrorCommand;
import ctc.tdcs.command.ScheduleCommand;
...
public class TdcsToolbarFactory {
	...
	public static void setDistrictName(String districtName) {
		TdcsToolbarFactory.districtName = districtName;
	}
	...
	private  CoolBar createCoolBar(Shell shell) {
		...
		/**xbm2010-4-20添加判断条件*/
		if(! isRoleFlag()){//对于教师，标记为true，不显示以下内容
			//当前时间和虚拟时间提示
			item2 = new CoolItem(coolbar, SWT.NONE);
...
/*	private  Control createTimeStepText(Composite composite) {
	...
	*/
	...
	private  Control createCombo(final Composite composite) {
		...
		/**xbm2010-4-20添加判断条件*/
		if( isRoleFlag()){//对于教师，标记为true，需要从数据库获取所有已经定义好的区段名称信息
			//从库中获取所有区段信息
			//hu 2010-11-2 修改
			//String [] items = BaseParam.getAllDistrictName();
			String [] items = new String[]{"实验区段"};
	...
	private  Control createToolBar(Composite composite) {
		...
		*/
		...
		scheduleItem = createItemHelper(toolBar,SCHEDULE, "设置并向各组所有学员发送调度信息");
		scheduleItem.setText("发送调度命令");
		scheduleItem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				ScheduleCommand dlgX = new ScheduleCommand(shell);//TdcsLayoutGraph_x.getPShell()
				dlgX.open();
			}
		});

		errorItem = createItemHelper(toolBar,ERROR, "设置并向各组所有学员发送故障信息");
		errorItem.setText("故障注入");
		errorItem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				ErrorCommand dlgX = new ErrorCommand(shell);//TdcsLayoutGraph_x.getPShell()
				dlgX.open();
			}
		});
		/**xbm2010-4-20添加判断条件*/
		if(! isRoleFlag()){//教师标记为true，不显示以下内容
	...
	private void initTDCS(){
		...
		scheduleItem.setEnabled(true);
		errorItem.setEnabled(true);
	}
